...
export default function SalePage() {
  ...
  useEffect(() => {
    async function fetchProducts() {
      try {
        // Try sorting by newest as fallback for sale items
        const res = await fetch("/api/products?sort=newest&limit=20");
        if (res.ok) {
          const data = await res.json();
          // Filter for products with a discounted compare_at_price
          const onSale = (data.products || []).filter(
            (p: Product) => p.compare_at_price && p.compare_at_price > p.price
          );
          setProducts(onSale);
        }
      } finally {
      ...
      <section className="relative py-24 px-4 text-center overflow-hidden">
        ...
        <div className="max-w-3xl mx-auto relative">
          <motion.p
            ...
            className="text-sm uppercase tracking-[0.3em] text-red-400 mb-4"
          >
            Limited Time
          </motion.p>
          <motion.h1
